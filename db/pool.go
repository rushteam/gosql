...
var defaultCluster *PoolCluster
...
//Open ..
func (c PoolCluster) Open(name, node string) (*sql.DB, error) {
	var dsn string
...
//Init ..
func Init(dbType string, settings map[string]map[string][]string) *PoolCluster {
	c := &PoolCluster{}
	c.dbType = dbType
	c.settings = settings
